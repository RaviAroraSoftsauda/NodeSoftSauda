...
//     if( req.query.id ) {
//         party.findById(req.query.id, 'contact_group', function(err, party){
//             console.log(party);
...
router.get('/saudaname', function(req, res) {
   ...
   console.log(br_code);
    delveryentry2.find({typ:"BR",pcode:br_code}, function (err, delveryentry2) {
          console.log(delveryentry1);
        if (err) {
            console.log(err);
        } else {
            res.json({ 'success': true, 'delveryentry2': delveryentry2,'delveryentry1': delveryentry1 });
        }
}).populate({path: 'sauda1 sl_code'} )
});
    ...
    router.get('/delivery_entry_list', ensureAuthenticated ,function(req,res){
       ...
       router.get('/delivery_entry_list_update/:id', ensureAuthenticated, function(req, res){
        ...
        router.get('/delete_delivery_entry/:id', function(req, res){
            if(!req.user.id)
            {
                res.status(500).send();
            }
